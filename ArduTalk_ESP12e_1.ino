...
String scan_network(void){
    ...
    AP_List += "<option value=\"\" disabled selected>請選擇</option>";
...
void handleRoot(int retry){
  ...
  temp += "<input type=\"serverIP\" name=\"serverIP\" value=\"140.113.199.200\" style=\"width: 150px; font-size:16px; color:blue;\" required>";
  ...
  temp += "</div></form><br>";
//  temp += "<center><span style=\" font-size:30px; color:blue; margin:0px; \"> 殺菌光遙控 </span></center>";
//  temp += "<center><div><input type=\"button\" value=\"ON\" style=\"width:100px; height:100px; background-color:#66CC66; font-size:38px; color:#4444FF; margin-right:50px; \" onclick=\"location.href=\'turn_on_pin\'\">";
//  temp += "<input type=\"button\" value=\"OFF\" style=\"width:100px; height:100px; background-color:#CC4444; font-size:38px; color:#4444FF;\" onclick=\"location.href=\'turn_off_pin\'\"></div></center>";
  temp += "</body></html>";
...
void loop() {
    ...
    if (millis() - cycleTimestamp > 200) {
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(14, result.toInt());
        }

        result = pull("D6");
        if (result != "___NULL_DATA___"){
            Serial.println ("D6: "+result);
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(12, result.toInt());
        }
        
        result = pull("D7");
        if (result != "___NULL_DATA___"){
            Serial.println ("D7: "+result);
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(13, result.toInt());
        }

         result = pull("D8");
        if (result != "___NULL_DATA___"){
            Serial.println ("D8: "+result);
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(15, result.toInt());
        }
